...
def hierarchical_clustering_foresight(nodes, edges, n_communities_final):
    n_nodes = len(nodes)
    n_edges = len(edges)
    ...
    for pair in similarity:
        if n_communities == n_communities_final:
            break
    ...
    return format_communities(deepcopy(community)), modularity
